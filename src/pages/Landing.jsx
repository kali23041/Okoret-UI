...
export default function Landing() {
  const terminalScript = [
    '$ echo "Developer: Kartik Sabale"',
    'Developer: Kartik Sabale',
    '$ echo "Portfolio: Kartikfx.cloud"',
    'Portfolio: Kartikfx.cloud',
  ]
  const fullTerminalText = terminalScript.join('\n')
  const [typedCount, setTypedCount] = useState(0)
  useEffect(() => {
    if (typedCount >= fullTerminalText.length) return
    const timer = setInterval(() => {
      setTypedCount((count) => (count < fullTerminalText.length ? count + 1 : count))
    }, 25)
    return () => clearInterval(timer)
  }, [typedCount, fullTerminalText.length])
  const typedText = fullTerminalText.slice(0, typedCount)
      ...
      <main className="relative">
        ...
        <section className="max-w-7xl mx-auto px-6 pb-16">
          <div className="mx-auto w-full max-w-md">
            <div className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-950/70">
              <div className="flex items-center gap-1 border-b border-neutral-900 bg-neutral-950/60 px-3 py-1.5">
                <span className="h-2 w-2 rounded-full bg-[#ff5f56]" />
                <span className="h-2 w-2 rounded-full bg-[#ffbd2e]" />
                <span className="h-2 w-2 rounded-full bg-[#27c93f]" />
                <span className="ml-2 text-[10px] text-neutral-500">zsh — Okoret</span>
              </div>
              <pre className="px-3 py-2 text-[11px] leading-relaxed text-neutral-300 font-mono whitespace-pre-wrap"><code>{typedText}<span className="ml-0.5 inline-block h-3 w-px align-[-1px] bg-neutral-500 animate-pulse" /></code></pre>
            </div>
          </div>
        </section>
      </main>
